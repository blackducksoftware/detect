...
* (IDETECT-4697) The Bazel detector has been replaced by the new Bazel V2 detector. The V2 detector supports both BZLMOD (MODULE.bazel) and WORKSPACE-based projects, performs automatic mode detection, and probes the dependency graph to determine which dependency sources are present. See [Bazel support](packagemgrs/bazel.md) for details.
...
* (IDETECT-4697) The `detect.bazel.workspace.rules` property has been removed and replaced by `detect.bazel.dependency.sources`. If present in your configuration, the old property will be silently ignored. See [Bazel support](packagemgrs/bazel.md) for migration details.
