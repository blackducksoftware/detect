...
public class BazelV2DetectableFatalWorkspaceTest {
    ...
    public void extract_whenBazelReportsFatalWorkspaceMisconfiguration_throwsDetectableExceptionWithoutHanging() throws Exception {
        ...
        // Every Bazel invocation returns the same fatal signature Bazel itself emits when a
        // local_repository/git_repository rule points at a location with no MODULE.bazel,
        // REPO.bazel, or WORKSPACE file (see TC-28 / BDSCA hang repro).
        ...
        // Bazel's own graph probing runs several probes (maven_install, maven_jar, haskell, http);
        // once the fatal signature is observed on the first call, every later probe must short-circuit
        // instead of invoking Bazel again — this is what prevents the hang seen in the real repro.
