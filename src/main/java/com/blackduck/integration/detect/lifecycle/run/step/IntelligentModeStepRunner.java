...
public class IntelligentModeStepRunner {
    ...
    public void runOnline(
        ...
        stepHelper.runToolIfIncluded(DetectTool.SIGNATURE_SCAN, "Signature Scanner", () -> {
            ...
            codeLocationAccumulator.addWaitableCodeLocations(signatureScannerCodeLocationResult.getWaitableCodeLocationData()); // null notification task range
        ...
        stepHelper.runAsGroup("Wait for Results", OperationType.INTERNAL, () -> {
            ...
            // in various reports.
            CodeLocationResults codeLocationResults = calculateCodeLocations(codeLocationAccumulator, codeLocationsUploadStartTime); // note: *** creates "true" notification range that may or may not be used later. otherwise, all we need is names of all code locations.
            codeLocationResults.getCodeLocationWaitData().setNotificationRange(codeLocationsUploadStartTime); // we set the range here because we want to be sure to include any notifications that may have been generated by the code location calculation step.
    ...
    public void runImpactAnalysisOnline(
        ...
        CodeLocationCreationData<ImpactAnalysisBatchOutput> uploadData = operationRunner.uploadImpactAnalysisFile( // note: we never make use of uploadData's notificationTaskRange field. it is null with the changes. should do better than null in future iteration.
            impactFile,
            projectNameVersion,
            impactAnalysisName,
